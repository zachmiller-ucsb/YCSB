...
public class RocksDBClient extends DB {
  ...
  static final long SLEEP = 30000; // 30000
  ...
  private RocksDB initRocksDBWithOptionsFile() throws IOException, RocksDBException {
    ...
    OptionsUtil.loadOptionsFromFile(config, optionsFile.toAbsolutePath().toString(), options, cfDescriptors);
    dbOptions = options;
      ...
      try {
        if (references == 1) {
          ...
          for (final ColumnFamily cf : COLUMN_FAMILIES.values()) {
            try {
              System.err.println(rocksDb.getProperty(cf.getHandle(), "rocksdb.stats"));
            } catch (Exception e) {
              e.printStackTrace();
            }
            cf.getHandle().close();
